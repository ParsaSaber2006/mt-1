<template>
  <div>
    ...
    <call-dialogue @onDialogLeave="showCallingDialog = false" v-if="advisor.id && showCallingDialog" :advisor="advisor"
      :selected_call_time="selected_call_time" @close="showCallingDialog = false" :category_id="this.category?.id" />
    ...
    <v-container v-if="!loading">
      <v-row class="pt-5">
        <v-col cols="12" sm="6" md="8">
          <v-card :elevation="elevation" class="pt-8 rounded-xl" style="overflow: visible !important">
            <div class="" style="
                position: absolute;
                top: 10px;
                right: 15px;
            ...
            <v-card-text>
              <v-container>
                <v-row class="justify-center">
                  <v-col cols="12" md="2" class="bordered text-center px-5 ">
                    <div class="text-center w-100 pa-10 pa-md-0">
                      <v-img :src="advisor.img_url" class="rounded-xl border border-avatar bg-white" cover
                        style="z-index: 10000;"></v-img>
                    </div>

                    <h1 class="advisor-title mt-3">
                      <strong> {{ advisor.title }} </strong>&nbsp;
                      <span class="">{{ advisor.name }}</span>
                    </h1>
                    <h3 class=" advisor-subtitle mt-1">
                      {{ advisor.job_title }}
                    </h3>
                    ...
                    <div class="mt-3 text-center">
                      <v-chip color="yellow-darken-4" append-icon="mdi-star" class="me-1 mb-2 ">{{ advisor.score_avg }} از
                        5</v-chip>
                      <v-chip color="green-darken-4" append-icon="mdi-bell-ring" class="px-4">{{ advisor.advises_count }}</v-chip>
                    </div>
                  </v-col>
                  <v-col cols="12" md="10" class=" ">
                    <div class="bosrder-s  px-10">
                      <div class="py-2 text-grey text-justify ">
                        {{ advisor.description }}
                      </div>
                      <div class="px-5 advisor-about text-justify" v-html="advisor.about"></div>
                ...
                <v-row justify="center" class="mt-15 1border-t">
                  <v-col cols="12">برنامه زمانی پاسخگویی: </v-col>
                  <template v-for="(h, day) in schedules" :key="day">
                    <v-col cols="6" sm="5" md="3" lg="2" v-if="h.length > 0">
                      <v-card elevation="0" class="border border-primary border-md h-100 rounded-xl">
                        <div class="text-center py-2 bg-grey-lighten-4 text-primary">
                          <strong>
                            {{ weekDays[day] }}
                          </strong>
                        </div>
                        ...
                        <v-card-text>
                          <div class="text-center">
                            <v-chip class="me-1 my-1 bg-grey-lihten-4" v-for="hour in h" :key="hour">
                               {{ hour[0] }} تا {{ hour[1] }}
                            </v-chip>
                          </div>
                        </v-card-text>
        ...
        <v-col cols="12" sm="6" md="4">
          <v-card :elevation="elevation" class="pb-10 rounded-xl" style="overflow: visible">
            <div class="text-center w-100" style="position: absolute; margin-top: -20px">
              <v-icon class="bg-green rounded-circle" style="
                  padding: 15px;
                  rotate: 270deg;
                  z-index: 10000;
                  margin-bottom: -28px;
            ...
            <v-card-text class="pa-5">
              <form>
                <div class="py-3">
                  <span class="text-subtitle-1 text-grey-darken-3">انتخاب مدت زمان مکالمه</span>
                </div>
                <div>
                  <v-radio-group v-model="selected_call_time">
                    <v-radio :value="time" v-for="time in [10, 30, 60]" :key="time">
                      <template v-slot:label>
                        <div>
                          <span class="text-h6 text-green-darken-2">{{ time }} دقیقه</span> -
                          <span class="text-info">({{ (advisor.credit_ratio * time +
                            advisor.entrance_fee).toLocaleString() }} تومان)</span>
                        </div>
                      </template>
                    </v-radio>
                  </v-radio-group>
                </div>
                <div v-if="!auth.isLogedIn" class="text-center text-grey-darken-2">
                  برای ارتباط با مشاور ابتدا باید وارد شوید. <v-btn @click="auth.showLoginForm()" variant="outlined"
                    append-icon="mdi-login" color="primary" size="small" rounded>وارد شوید</v-btn>
                </div>
                <div v-if="auth.isLogedIn" class="text-subtitle-1 text-grey-darken-3 text-center">
                  اعتبار شما:
          ...
          <v-card :elevation="elevation" class="mt-5 pa-2 rounded-xl" style="overflow: visible">
...
export default {
  components: { RequestForCall, CallDialogue, Comments },
  metaInfo() {
